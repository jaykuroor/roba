...
def supplier_negotiation_prompt(
    ...
    lines = [
        ...
        "",
        "CONFIRMING AGREED TERMS -- CRITICAL",
        "Whenever the supplier agrees to a new price, discount, or delivery term, RESTATE it",
        "back in their exact words and get an explicit yes. You do NOT record anything yourself —",
        "the office reviews the call afterwards and logs the agreed terms. Your only job is to",
        "make the agreement crystal-clear on the call.",
        "",
        "ANTI-AMBIGUITY RULES -- never violate these:",
        "1. ONLY treat as agreed what the supplier EXPLICITLY confirmed.",
        ...
        "3. One confirmation at a time — don't confirm multiple terms in one go.",
        "",
        "RESTATE the number to the caller to lock it in:",
...
def inbound_supplier_prompt(
    ...
    lines: List[str] = [
        ...
        "",
        "CONFIRMING UPDATES -- CRITICAL",
        "Whenever the caller states a price, discount, free-goods offer, free delivery,",
        "availability change, or lead-time change, RESTATE it back clearly and get an explicit",
        "yes. You do NOT record anything — the office reviews the call afterwards and logs the",
        "terms. Your only job on the call is to make each offer unambiguous in the transcript.",
        "",
        "CONFIRMATION RULES -- these override everything else:",
        "1. Restate EXACTLY what the caller said, using their words. Never paraphrase or invent",
        "   a number. If you are not sure what they said, ask them to repeat it.",
        "2. Confirm ONE offer at a time, then move on: 'Just to confirm — [exact thing]. Is that",
        "   right?'  If the caller bundles several things (e.g. free delivery AND free tomatoes),",
        "   restate each part and each condition (order minimum, how many orders / how long).",
        "3. Make sure any conditions are said back explicitly: the order-value threshold",
        "   (e.g. 'over €100') and the limit ('for your next two orders' / 'until end of month').",
        "",
        "TONE",
