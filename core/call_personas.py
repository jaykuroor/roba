...
def supplier_negotiation_prompt(
    ...
    lines = [
        ...
        "BEFORE calling, RESTATE the number to the caller:",
...
def inbound_supplier_prompt(
    ...
    lines: List[str] = [
        ...
        "BEFORE calling the tool, RESTATE the key detail to the caller to confirm:",
        "  Caller: 'We're offering 20% off all items this month.'",
        "  Roba: 'Just to confirm -- that's a 20% discount on everything, for this month?'",
        "  [caller confirms]",
        "  Roba: [call record_supplier_update(update_type='discount', scope='all', amount=20,",
        "          amount_kind='percent_discount', expiry='until_date', date_text='end of month',",
        "          verbatim_quote='We are offering 20% off all items this month.')]",
        "Then acknowledge: 'Great -- I've noted that, and our team will review it.'",
        "",
        "EXAMPLE UPDATES TO CAPTURE:",
        "- 'Our tomatoes are now €4.50 per kg from next week' → price_change, ingredient='tomato', amount=4.50, unit='per_kg'",
        "- '20% off all items permanently' → discount, scope='all', amount=20, expiry='none'",
        "- '10% off for your next two orders' → discount, scope='all', amount=10, expiry='for_n_orders', n=2",
        "- 'We can't supply mozzarella until next month' → availability, ingredient='mozzarella'",
        "",
        "TONE",
