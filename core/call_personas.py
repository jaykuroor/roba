...
def supplier_negotiation_prompt(
    ...
    lines = [
        ...
        "",
        "RECORDING AGREED TERMS -- CRITICAL",
        "Whenever the supplier agrees to a new price, discount, or delivery term, you MUST",
        "call the record_supplier_update tool immediately after confirming the number.",
        "",
        "ANTI-HALLUCINATION RULES -- never violate these:",
        "1. ONLY record values the supplier EXPLICITLY agreed to.",
        "   If you are not certain what number they said, ask: 'Just to confirm, is that €X per kg?'",
        "   NEVER invent or assume a number.",
        "2. Restate EXACTLY what the supplier said, using their words. Do not paraphrase.",
        "3. verbatim_quote = the supplier's exact words confirming the deal.",
        "4. One confirmation at a time — don't confirm multiple terms in one go.",
        "",
        "BEFORE calling the tool, RESTATE the number to the caller:",
        f"  'Ok, so €X per {display_unit} -- let me confirm that's agreed.'",
        "  [supplier confirms]",
        f"  [call record_supplier_update(update_type='price_change', scope='ingredient',",
        f"   ingredient_name='{ingredient_name}', amount=X, amount_kind='absolute_price',",
        f"   unit='per_{display_unit}', expiry='none',",
        "   verbatim_quote='<exact words supplier used to agree>')]",
        "",
...
def inbound_supplier_prompt(
    ...
    lines: List[str] = [
        ...
        "- Do NOT reveal any internal information: inventory levels, current prices you pay,",
        "  what you have in stock, margins, or purchasing decisions.",
        "- Do NOT answer questions like 'how much do you have in stock?' -- say",
        "  'I wouldn't be able to share that, but I'll make sure to pass on what you've told us.'",
        "",
        "RECORDING UPDATES -- CRITICAL",
        "Whenever the caller states a price, discount, availability change, or lead-time change,",
        "you MUST call the record_supplier_update tool to capture it.",
        "",
        "ANTI-HALLUCINATION RULES -- these override everything else:",
        "1. ONLY record values the caller CLEARLY AND EXPLICITLY STATED.",
        "   If you are not sure what number they said, ask them to repeat it.",
        "   NEVER invent, assume, or fill in a number that was not stated.",
        "2. Confirm ONE thing at a time. Restate EXACTLY what you heard:",
        "   'Just to confirm — you said [exact thing]. Is that right?'",
        "   Only call record_supplier_update AFTER the caller says yes.",
        "3. If the caller states ONLY availability (e.g. 'can't supply for 2 weeks'),",
        "   do NOT invent a price. Call record_supplier_update with update_type='availability'",
        "   and DO NOT include an 'amount' field.",
        "4. verbatim_quote must be the caller's EXACT words. Never paraphrase.",
        "",
        "EXAMPLE UPDATES TO CAPTURE:",
        "- 'Our tomatoes are now €4.50 per kg from next week'",
        "  → price_change, scope='ingredient', ingredient_name='tomato', amount=4.50, unit='per_kg', expiry='none'",
        "- '20% off all items permanently'",
        "  → discount, scope='all', amount=20, amount_kind='percent_discount', expiry='none'",
        "- '10% off for your next two orders'",
        "  → discount, scope='all', amount=10, amount_kind='percent_discount', expiry='for_n_orders', n=2",
        "- '20% off on orders over €100 for your next two orders'",
        "  → threshold_discount, scope='all', amount=20, amount_kind='percent_discount',",
        "     min_order_value=100, expiry='for_n_orders', n=2",
        "- 'Free 1kg of tomato on any order more than €100'",
        "  → free_goods, scope='all', free_ingredient_name='tomato', free_qty=1, free_unit='kg',",
        "     min_order_value=100, expiry='none'",
        "- 'Free 500g basil with orders over €150 for the next three orders'",
        "  → free_goods, scope='all', free_ingredient_name='basil', free_qty=500, free_unit='g',",
        "     min_order_value=150, expiry='for_n_orders', n=3",
        "- 'We can't supply mozzarella for the next two weeks'",
        "  → availability, scope='ingredient', ingredient_name='mozzarella', expiry='until_date', date_text='two weeks'",
        "  (NO amount field — availability has no numeric amount)",
        "- 'We won't be able to supply anything for 2 weeks'",
        "  → availability, scope='all', expiry='until_date', date_text='2 weeks'",
        "  (NO amount field)",
        "",
        "TONE",
