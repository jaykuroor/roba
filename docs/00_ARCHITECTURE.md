...
- **Apply step (deterministic per intent):** e.g. `set_leave` → write `attendance`/shift rows for the window + emit `STAFF_COVERAGE` impact via Staff agent's domain (Track A consumes the `USER_FACT`); `record_receipt` → create `inventory_lot` + `inventory_ledger(receipt)`; `add_event` (e.g., "parade Monday") → store as a `USER_FACT` with an `effective_window` and a demand multiplier tag the Forecaster reads (§18.4); `add_menu_item` → create `menu_items` row, then LLM drafts a `recipe` (validated, §12) ; `add_inventory_count` → `inventory_levels.last_counted_*` + reconciliation ledger entry.
